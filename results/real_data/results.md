...
The HMM-MoA posterior JSON (`demo/hmm_moa/posterior_trace.json`) now includes
`agent_mixture` weights with the corresponding agent labels as well as `agent_Qs`
for the per-agent action values. This makes it straightforward to quantify the relative
contribution of the Model-free (value and choice channels separately), Model-based,
and Bias components for each trial and to reproduce the agent-specific trial-history regressions.
