...
def build_posterior_payload(label, extras, batch, metrics, agent_labels=None):
    ...
    agent_Qs = extras.get("agent_Q")
    if agent_Qs is not None:
        payload["agent_Qs"] = tensor_first_sequence(agent_Qs).tolist()
...
def evaluate_series(model, batch, agents=None):
    ...
    if agents is not None:
        agent_Q = build_Q_seq(
            batch["actions"], batch["rewards"], batch["transitions"], agents
        )
    extras = {
        "gamma": gamma,
        "gating": gk,
